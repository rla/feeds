...
var markFeedSeen = read.prepare('UPDATE article SET is_seen = 1 WHERE feed = ?');
var markImportant = read.prepare('UPDATE article SET is_important = ? WHERE uuid = ?');
var markSeen = read.prepare('UPDATE article SET is_seen = ? WHERE uuid = ?');
...
// Retrieves unseen articles.

exports.unseen = function(start, count, cb) {
    var sql = 'SELECT * FROM article_with_feed WHERE is_seen = 0 ORDER BY published DESC LIMIT ? OFFSET ?';
    read.all(sql, [ count, start ], cb);
};
